...
// Per-fuel-type UI config and defaults
const FUEL_TYPES = {
    petrol: {
...
// Engine displacement (litres) → typical efficiency in Nigerian mixed driving.
// Values are conservative vs EU/US ratings: Nigerian heat, traffic, and road conditions
// reduce real-world efficiency by ~15-20% compared to official specs.
// null = that combination doesn't exist in practice.
...
function saveLedger() {
    try {
        localStorage.setItem(STORAGE_KEY, JSON.stringify({ trips: ledgerTrips, fuelType }));
    } catch {}
}
function loadLedger() {
    try {
...
function setFuelType(type) {
    ...
    // Reset fuel price to type default
    elFuelPrice.value = cfg.priceDefault;

    // Repopulate vehicle presets for this fuel type
    ...
    // Set efficiency to type default
    elManualEfficiency.value = cfg.effDefault.toFixed(1);

    // Engine size is meaningless for electric (no displacement)
    document.getElementById('engineSizeBlock').style.display = type === 'electric' ? 'none' : '';
    document.getElementById('engineSize').value = '';

    // Update fuel preview label (only when AC is off — toggleAC handles the AC-on case)
    if (!acOn) {
...
function handleEngineSizeSelect(displacement) {
    ...
    if (eff == null) {
        // No real-world mapping for this combo (e.g. 0.8L diesel doesn't exist in Nigeria)
        document.getElementById('engineSize').value = '';
        return;
    }
...
function handleSliderInput(value) {
    ...
    Object.entries(PLATFORM_PRESETS).forEach(([id, p]) =>
        document.getElementById(id).classList.toggle('active', p === v));
    updateCurrentTripPreview();
}
...
function calcTrip() {
    ...
    const units      = effRate > 0 ? distance / effRate : 0; // litres, kg, or kWh
...
function addTrip() {
    ...
    ledgerTrips.push({ id: Date.now(), acWasOn: acOn, fuelType, ...s });
...
function renderLedger() {
    ...
    ledgerTrips.forEach((trip, idx) => {
        ...
        const acBadge      = trip.acWasOn ? `<span class="badge badge-ac">AC</span>` : '';
...
function calcDailyTotals() {
    ...
    const hourlyWage  = totalHours > 0 ? totalNet / totalHours : 0;
    ...
    const costPerKm   = totalDist > 0 ? (totalPlatform + totalFuel + totalMaint) / totalDist : 0;
    const profPerKm   = totalDist > 0 ? totalNet / totalDist : 0;
    const shiftMargin = totalGross > 0 ? (totalNet / totalGross) * 100 : 0;
    const maintRate   = parseFloat(elMaintRateInput.value) || 0;
    const unitLabel   = FUEL_TYPES[fuelType].unitLabel;

    animateProfit(totalNet);
    ...
    document.getElementById('bdNet').textContent           = fmt(totalNet);
...
// ── Init ──
loadLedger();
setFuelType(fuelType); // populates vehicle presets and syncs all labels
updateCurrentTripPreview();
renderLedger();
calcDailyTotals();
